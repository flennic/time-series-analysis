...
**Answer:** Looking at the ACF plot we see a decreasing correlation over time. As the correlation is continuingly following a downwards trend, while all of the lags keep being statistically significant, we should have a closer look at the first difference. He we see a spike at lag 1, as well as an indicator for the seasionality with a lag of 12. The second lag is significant, but the typical AR ACF plot indicates an decay, so we choose 1 for the AR part. Looking at the PACF plots we can observe the seasionality again, also we see a significant spike at lag 1. Therefore we choose the following model: $\text{ARIMA}(1, 1, 1)\times(0, 0, 1)_{12}$.
...
**Answer:** Let's look at different plots first and then do some interpretation.

**so2:** We see again that is makes sense to take the first difference for the ACF plot, as all spikes seem to be significant. The remaining two spikes, which are contrary in direction, indicate an MA process, which can also be seen by the PACF with the first difference. It does not seem that there is a seasionality, so the chosen model is $\text{ARIMA}(0, 1, 1)$ or $\text{ARIMA}(0, 1, 2)$.

**EQcount:** We can see the typical behaviour of an AR process in the ACF plot, which is quickly decaying and we have no further increase in spikes. So we assume an underlying AR process. As we observe no seasionality, nor a major change in the differnce plots, we chose the model $\text{AR}(1)$.

**HCT:** The ACF plot of the first difference shows a slowly decaying difference, while having recurring spikes. As the spikes are separated by 7 lags, we assume a weekly seasionality. The PACF shows significance at lag 3, as well as a significane for lag 2 at the first difference. Therefore we suggest the model $\text{ARIMA}(7, 1, 1)\times(0,0,1)_7$.
