...
rhine_data_wide <- dcast(rhine_data_wide, formula = TotN_conc+Year+Time~Month, value.var = "dummy", fill = "0")

lm_model_month_lag <- lm(data=rhine_data_wide, 
                    TotN_conc~Month_1+Month_2+Month_3+Month_4+Month_5+Month_6+Month_7+
...
lm_model_month_lag_step <- stepAIC(lm_model_month_lag, scope = list(upper = ~Month_1+Month_2+
...
df$smooth_one_week_lag <- ksmooth(x = df$oil_price_one_diff, y = df$gas_price_one_diff, bandwidth = 0.4, kernel = "normal")$y
df$smooth_two_week_lag <- ksmooth(x = df$oil_price_two_diff, y = df$gas_price_two_diff, bandwidth = 0.4, kernel = "normal")$y
df$smooth_three_week_lag <- ksmooth(x = df$oil_price_three_diff, y = df$gas_price_three_diff, bandwidth = 0.4, kernel = "normal")$y
...
\fbox{\begin{minipage}{46.7em}
e) Fit the following model: $y_t = \alpha_0 + \alpha_1 I(x_t > 0) + \beta_1 x_1 + \beta_2 x_{t-1} + w_t$ and check which coefficients seem to be significant. How can this be interpreted? Analyze the residual pattern and the ACF of the residuals.
\end{minipage}}
